// Generated from https://html.spec.whatwg.org/entities.json — do not edit by hand.
// All 2125 named character references: "name:hexCodePoint", a leading "*"
// marking the 106 names HTML also accepts without the trailing semicolon
// ("&nbsp", "&copy"), a "." joining the 93 references that map to two code
// points ("NotEqualTilde:2242.338").
//
// Kept complete on purpose. HTML consumes the *longest* reference it knows, so a
// partial table would not just miss names — it would mis-decode them: without
// "notin;" in the table, "&notin;" matches the legacy "&not" and comes out as
// "¬in;" instead of "∉".
const TABLE =
  ...
  'emptyset:2205 emptyv:2205 emsp:2003 emsp13:2004 emsp14:2005 eng:14b ensp:2002 eogon:119 eopf:1d556 epar:22d5 ' +
  ...
  'sup:2283 *sup1:b9 *sup2:b2 *sup3:b3 supE:2ac6 supdot:2abe supdsub:2ad8 supe:2287 supedot:2ac4 suphsol:27c9 ' +
...
/** Keys include the trailing ";" where HTML requires it. */
export const ENTITY_KEYS = new Map<string, string>();

/** Longest key in the table, so the decoder knows how far back to try. */
export const MAX_ENTITY_KEY_LEN = 32;

for (const entry of TABLE.split(' ')) {
  const sep = entry.indexOf(':');
  const legacy = entry[0] === '*';
  const name = entry.slice(legacy ? 1 : 0, sep);
  const chars = String.fromCodePoint(...entry.slice(sep + 1).split('.').map(h => parseInt(h, 16)));
  ENTITY_KEYS.set(name + ';', chars);
  if (legacy) ENTITY_KEYS.set(name, chars);
}
