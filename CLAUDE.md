...
- **`decodeEntities()` — детали:** таблица `ENTITY_TABLE` сгенерирована из `https://html.spec.whatwg.org/entities.json` и урезана до блоков, встречающихся в заголовках (ASCII, Latin-1, Latin Ext-A, греческий, пунктуация, валюты, letterlike) — 359 имён / 465 форм, ~5 КБ; полный HTML5-набор стоил бы 16 КБ ради `&boxDL;`. Формат `name:hex`, префикс `*` = имя, которое HTML принимает и без `;`. Регистр значим (`&Eacute;` ≠ `&eacute;`). Один проход (`&amp;lt;` → `&lt;`, не `<`); неизвестные имена (`AT&T`, `&boxDL;`) остаются как есть; числовые ссылки из C1-диапазона ремапятся по-браузерному (`&#146;` → `’`). ⚠️ Тестировать через DOM нельзя: linkedom не декодирует сущности вообще — тесты сверяются с эталонным JSON
