...
| HTML | Markdown |
| --- | --- |
| `<h1>`…`<h6>` | `#`…`######`, shifted by `headingOffset` or by `topHeadingLevel` — **neither in the library, `topHeadingLevel: 3` in the extension** — and clamped to 1…6 |
| `role="heading"` with `aria-level` | the same, at the level stated — an interface built out of divs writes its headings this way — but only where the page *drew* one. A tag needs no such test, since the browser paints an `<h3>` large and bold whatever the page says; a `<div>` is painted like body text, so the role is a claim about meaning and the drawing is the only evidence for it. Three things have to agree: the element takes a line of its own (a role inside a sentence is a label, and `##` would cut the sentence in two), it holds no block of its own (a role on the wrapper of a whole section is not its heading), and it was drawn larger or heavier than the text around it. The third is asked only where a style snapshot answered it — the library has none, and there the role stands alone as before. A missing or unreadable level reads as 2, which is what a browser reports for a heading that does not say; a level below ARIA's floor of 1 is unreadable too, while one past 6 is a level the page really stated and is clamped, like a tag shifted past the bottom. Counts for `topHeadingLevel` like a heading tag — and a role written as a paragraph counts for nothing, or it would set a level no line of the file is at |
...
| `<ol start="5">` | numbering continues from `5.`; `0` is legal and kept, and a `start` no number can be read out of numbers from 1, as a browser renders it. A **negative** start is legal in HTML and has no marker in Markdown — a CommonMark ordered marker is digits — so those items keep the numbers the page drew and become a block each; written as items they were one paragraph with every line welded onto the last. Two lists the page drew apart alternate `.` and `)`, because a blank line does not end a list: `<ol start="9">` under a list that had ended at 2 was swallowed by it and renumbered 3, and the `9.` the page showed was gone from the rendered file while the source still spelled it |
| `<li>` with a checkbox | `- [x]` / `- [ ]` — the item's **own** box, never one belonging to an item nested under it: a plain parent holding a task list inherited its first child's state and the file claimed *done* about an item the page never marked |
...
| a language bar drawn *beside* the `<pre>` | moved into it as the `<figcaption>` the page could have written, so the language becomes the info string and the control goes with the rest of the furniture. `<div class="code-block"><div><span>python</span><button>Копировать</button></div><pre>…` is ChatGPT, Habr and half the documentation sites, and it arrived as a paragraph reading `pythonКопировать` above a fence with no language at all. Narrow on purpose: only where the wrapper holds the bar and one `<pre>` and nothing else — its own text counts, so `<div>Intro <div>python</div><pre>…` is not this shape — only where what is left of the bar once its controls are gone is a language a highlighter really ships, and never a heading at any depth in the bar, `role="heading"` included: an `<h3>` above a sample is the author's, and `<h3>HTTP</h3>` over a request is a section named for its language rather than a widget naming one |
...
| `<pre><code data-lang="js">` | ` ```js ` fence — also from `data-language`, from a highlighter class (`language-js`, `lang-js`, `highlight-source-js`, `brush: js`, `sourceCode js`, `shj-lang-js`, `prettyprint lang-js`) on the `<code>`, the `<pre>` **or the box around it** — GitHub states it there and nowhere else (`<div class="highlight highlight-source-typescript">`), so a sample copied out of a repository had been arriving with no language at all; the bare-name spelling below is never read off a wrapper, which is where `highlight`, `snippet` and `code-block` live and from a class that is the bare name of a language (`<code class="java">`, what highlight.js writes when the page states the language and what Habr's editor produces — matched against a list of real languages, so a `snippet` or a `highlight` class opens no fence with it); anything that is not a bare language token is dropped rather than written into the info string |
...
| `<table border="0">` with no `<th>`, `<thead>` or `<caption>` of its own | not a table at all: the cells become blocks, the scaffolding goes. That attribute is HTML 4's way of saying "draw no grid", which is what a page writes when the table is furniture — Hacker News builds a whole discussion page out of them, and read as grids it came back as 378 KB of pipes on six lines. `role="presentation"` says the same in the modern spelling; `role="table"` denies it |
| a table inside a layout table | asked the same questions on its own account, and answered before the outer table is taken apart: a data table keeps its grid, another layout table is flattened when its turn comes. Nesting is a signal neither way — a nested table used to be unwrapped along with the outer table's scaffolding, so a two-column table in a layout cell arrived as `NameAgeAnn30` and two Hacker News comments arrived on one line, and a header belonging to a nested table used to keep the outer one a grid |
...
| a background differing from the fill behind it | `==text==` | the tag is the older spelling; every editor with a highlighter button writes a `<span>` with a fill instead — Google Docs, Notion, Confluence — and a note whose marks all came from one arrived with none of them. `background-color` does not inherit, so what makes a fill a highlight is that it *differs* from the nearest painted ancestor, which only the content script can see: it records the colour where it is news and nowhere else. Never an image or a gradient (a decoration, not a mark); never a block, whatever it is painted — a card, a callout, a striped row and a code block are not marked phrases; never a control, which the browser paints and nobody marked; and never a fill under monospaced type, which is a code chip written by a page that means `<code>` (K7) |
...
| `<q>` | `“quoted”` | the marks a UA stylesheet draws, written as characters. The pair comes from the nearest `lang` — `«…»` under `ru`, `„…“` under `de`, `「…」` under `ja` — out of CLDR's delimiters, which is what CSS's `quotes: auto` resolves against; unknown falls back to `“…”`, and a nested `<q>` takes the second pair. An empty one writes nothing — decided rather than overlooked, and the one place this rule does not follow the screen: a browser draws both marks for `<q></q>` too, 14px of them, so the file loses something the reader met. It is kept that way because an empty `<q>` is almost always a template that rendered its variable to nothing, and `""` in the middle of a sentence reads as debris the page never showed. A page that styled the marks off (`quotes: none`) gets them anyway: that is a stylesheet, and the core reads attributes |
...
| LaTeX using no LaTeX — no `\`, `^`, `_`, `{`, `}`, `&` — **or** carrying a tag or a comment opener | the glyphs the page drew, as text | not a formula but text somebody ran through a renderer, and the screen settles it: `x <x-foo style="position:fixed">X</x-foo> y` is drawn `x custom X y`, and reading it as a formula put an attribute in the file that stood nowhere on screen. `E = mc^2` and `\frac{a}{b}` never reach this branch. The price, knowingly paid: on a real KaTeX page `a < b` and `x + y` now come back as `a\<b` and `x+y` — glyph spans carry no spaces |
| a renderer's wrapper carrying no LaTeX at all | the glyphs the page drew, as text | KaTeX with `output: "html"` builds no MathML, and MathJax v3 emits assistive MathML only with the a11y extension. The wrapper rules ask for a formula rather than for the class, or the whole subject leaves the page: `math: true` was writing nothing where `math: false` kept `E=mc2`. It is a fallback and never a source — glyphs read back off the screen are not the formula, and `\frac{a}{b}` comes back `ba` |
...
| a container whose content was drawn on **one line** | its items stay in that line. Counted rather than derived: the content script asks a `Range` how many bands the container's content occupies, and one band is one line whatever `flex-direction` says. It is what a flex **column** holding a single item always was — the item and the container are in the same place, and where that place is was settled higher up — so a mention wrapped in a box of its own inside a flex row no longer breaks the sentence it stands in. Only a `<div>`, `<section>`, `<article>` or `<main>` item is taken back into the line, and only when it holds no block of its own: a row of one-line cards measures as one band too, and those blocks must stay blocks. A heading keeps its level, a list keeps its bullets, a cell keeps its column — and a `<p>` keeps its paragraph, since it is the one tag that means only that, and the page's own word outweighs a band |
...
| `display: inline` on a heading | still a heading, unless something drew before it on its line. Wikipedia's Vector 2022 inlines every `<h2>` so its `[edit]` link lands beside the title, and reading that as "not a heading" cost an article all 60 of its sections |
...
| | |
| --- | --- |
| `topHeadingLevel: 3` | the shallowest heading of the capture is *raised* to `###` and the rest keep their distance from it — never pushed deeper, so a capture that already starts at `<h1>` keeps its `#` and one starting at `<h2>` keeps its `##`: those ranks are the ones the page gave its own title and sections. Not a fixed shift either: a chat interface writes a whole answer under `<h3>`, and shifting by one made a file whose first heading was `####` with nothing above it. Three rather than two because a clip is read where it lands — a note of one's own is titled by the front matter, but a clip pasted under a section of an existing note sits level with that note's own sections at `##` and under the section it was put in at `###`. Worked out per capture — across every fragment when there are several, so two ranks a highlighter picked out do not collapse into one — and carried across presses of the button by the panel, so a page captured in three goes is not three `###` with nothing under them |
...
| a partial selection is enriched first | `enrichRange()` gives back the table header row, the list's numbering, the code block's language and the block the range was cut out of — and the box a measured row was counted in: `ROW_ATTR` sits on the container, so a drag that starts and ends inside one made that container the common ancestor and `cloneContents()` handed back its children with the mark left on the page. Selecting the whole row was always right, which is what hid it; the broken gesture was the commoner one, dragging across the sentence itself, and it came back as three paragraphs. A `<pre>` cut into keeps whatever structure survived the cut, and is flattened to text only where none did: the caption naming the language and the copy button live *inside* the `<pre>` on Perplexity, so a block selected from within it came back as ```` ```pythonCopydef hello(name): ```` — both words of chrome welded onto the sample and the language gone from the info string. A selection lying inside one row leaves that row outside the clone, so it is rebuilt from the bare cells — and rebuilt fresh it carried none of the original's marks, the header mark included, so selecting the header's own cells restored the header beside them and the file held it twice. The mark travels from the row the cells were cut out of. A bar drawn *beside* the block is taken too, and only through `liftCodeHeaders` itself: a deep copy of the wrapper is lifted and the caption kept only if the lift produced one, so an author's own heading in that place — or a bar naming no language, or a wrapper holding text of its own — is refused exactly as it is for a whole capture. A task box is restored the way the ordinal is, off the original, since a drag over the text of a task starts after the `<input>` and `- [x] done` arrived `- done`. And the quotations above whatever the dispatcher picked are put back: it stops at the nearest ancestor, so a drag inside a list *in a quotation* took the list branch and `> - inner item` came back `- inner item` — the words kept and the fact that someone else said them gone |
| `\n` inside a text node → `<br>` | only where the computed `white-space` of the element holding it preserves the newline — `pre`, `pre-wrap`, `pre-line`, `break-spaces`. That is how Instagram and anything else that breaks lines inside one `<span>` gets its paragraphs; under `normal` the browser draws a space and so does the file, which is why an indented `<p>` no longer arrives with a hard break per source line. Skipped where the core keeps the whitespace itself (`pre`, `code`, `kbd`, `samp`, `textarea`) or reads the subtree raw (`script`, `style`, `svg`, math). A newline against a *node's* edge is trimmed only where nothing was drawn on that side — text, a `<br>`, or a replaced element like an image or a video, which paints a box `textContent` cannot see |
