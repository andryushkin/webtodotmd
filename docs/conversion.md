...
| LaTeX using no LaTeX — no `\`, `^`, `_`, `{`, `}`, `&` | the glyphs the page drew, as text | not a formula but text somebody ran through a renderer, and the screen settles it: `x <x-foo style="position:fixed">X</x-foo> y` is drawn `x custom X y`, and reading it as a formula put an attribute in the file that stood nowhere on screen. `E = mc^2` and `\frac{a}{b}` never reach this branch. The price, knowingly paid: on a real KaTeX page `a < b` and `x + y` now come back as `a\<b` and `x+y` — glyph spans carry no spaces |
