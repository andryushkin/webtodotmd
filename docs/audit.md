...
5. The `htmltodotmd` gitlink, its checkout, and its license.
6. Local license texts for every vendored dependency and their inclusion in
   the packaged extension.
7. Guide size budgets (`CLAUDE.md` at most 130 lines, `AGENTS.md` at most 45).
...
5. Is the pinned submodule commit reachable from its public remote?
...
7. Did a fresh recursive clone build without local paths or untracked files?
