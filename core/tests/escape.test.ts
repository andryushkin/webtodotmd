...
// Зеркало правила подсветки: раз `<mark>` пишет `==`, то и `==` со страницы
// стало разделителем — для всего, что этот маркер понимает. `x==y and C==C++` —
// предложение из сравнений и ни одной подсветки, и без экранирования читатель
// получал `y and C` жёлтым.
describe('литеральные ==', () => {
  const md = (html: string) => toMarkdown(html).trim();

  it('пара сравнений разбирается', () => {
    expect(md('<p>x==y and C==C++</p>')).toBe('x\\=\\=y and C\\=\\=C++');
  });

  it('одиночное сравнение не платит ничего', () => {
    expect(md('<p>if x==y then stop</p>')).toBe('if x==y then stop');
  });

  // Один `=` не маркер ни в одном диалекте, а `====` под заголовком — черта,
  // которую страница нарисовала на своей стороне и в тексте не показывала.
  it.each([
    ['одиночные', '<p>x = y and a = b</p>', 'x = y and a = b'],
    ['в конце строки', '<p>result ==</p>', 'result =='],
  ])('не трогает: %s', (_name, html, expected) => {
    expect(md(html)).toBe(expected);
  });

  it('подсветка рядом со сравнением: разбирается только текст', () => {
    expect(md('<p><mark>hit</mark> and x==y==z</p>')).toBe('==hit== and x\\=\\=y\\=\\=z');
  });
});
